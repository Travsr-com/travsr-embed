...
fn load_tract_model(dir: &Path) -> Result<Arc<TypedRunnableModel>> {
...
fn make_batches(texts: &[String]) -> Vec<Vec<String>> {
    texts.chunks(BATCH_CAP).map(|c| c.to_vec()).collect()
}
...
fn main() -> Result<()> {
    ...
    for &n in &[1usize, 2, 4, 8] {
        ...
        let speedup = if baseline_nps > 0.0 {
            nps / baseline_nps
        } else {
            1.0
        };
